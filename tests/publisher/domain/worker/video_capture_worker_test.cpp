...
namespace capture_support = semilive::publisher::test_support::capture;
...
std::unique_ptr<capture_support::ScriptedDesktopCaptureBackend>
scripted_backend(capture_support::ScriptedDesktopCaptureScript script) {
    return std::make_unique<capture_support::ScriptedDesktopCaptureBackend>(
        std::move(script));
}
...
void start_failure_returns_error_and_restores_idle() {
    ...
    capture_support::ScriptedDesktopCaptureScript script;
    publisher::DefaultVideoCaptureWorker worker{
        scripted_backend(std::move(script)), store, notifier};
...
capture_support::ScriptedDesktopCaptureScript fatal_script() {
    capture_support::ScriptedDesktopCaptureScript script;
    script.initial_width = 2;
    script.initial_height = 1;
    script.steps.emplace_back(capture_support::ScriptedDesktopCaptureFailure{
        capture_contract::DesktopCaptureIssue{
...
void runtime_failure_notifies_once_and_requires_stop() {
    ...
    publisher::DefaultVideoCaptureWorker worker{
        scripted_backend(fatal_script()), store, notifier};
...
capture_support::ScriptedDesktopCaptureScript recovery_script() {
    capture_support::ScriptedDesktopCaptureScript script;
    script.initial_width = 2;
    script.initial_height = 1;
...
void temporary_failure_reuses_pixels_and_records_recovery() {
    ...
    publisher::CapturedVideoFrameStore store{notifier, 64};
    publisher::DefaultVideoCaptureWorker worker{
        scripted_backend(recovery_script()), store, notifier};
...
capture_support::ScriptedDesktopCaptureScript unavailable_script() {
    capture_support::ScriptedDesktopCaptureScript script;
    script.initial_width = 2;
    script.initial_height = 1;
    const auto issue = capture_contract::DesktopCaptureIssue{
...
void recovery_timeout_becomes_a_runtime_failure() {
    ...
    publisher::DefaultVideoCaptureWorker worker{
        scripted_backend(unavailable_script()), store, notifier};
