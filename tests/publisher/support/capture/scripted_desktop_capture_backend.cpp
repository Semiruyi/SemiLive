#include "publisher/support/capture/scripted_desktop_capture_backend.hpp"
...
ScriptedDesktopCaptureBackend::ScriptedDesktopCaptureBackend(
    ScriptedDesktopCaptureScript script)
    : script_{std::move(script)} {
    if (script_.output_name.empty()) {
        throw std::invalid_argument{"scripted output name must not be empty"};
    }
    if (script_.initial_width == 0 || script_.initial_height == 0) {
        throw std::invalid_argument{"scripted output dimensions must be non-zero"};
    }
}

std::expected<contracts::capture::DesktopCaptureInfo, DesktopCaptureIssue>
ScriptedDesktopCaptureBackend::open(
    const contracts::capture::DesktopCaptureConfig& config) {
    if (open_) {
        return std::unexpected{
            issue(DesktopCaptureOperation::Open,
                  "scripted desktop capture backend is already open")};
    }
    if (config.output.selection == DesktopOutputSelection::Index &&
        config.output.index != 0) {
        return std::unexpected{
            issue(DesktopCaptureOperation::Open,
                  "scripted desktop output index is out of range")};
    }
...
DesktopCaptureResult ScriptedDesktopCaptureBackend::capture_latest() {
    if (!open_) {
        return std::unexpected{
            issue(DesktopCaptureOperation::Acquire,
                  "scripted desktop capture backend is not open")};
    }
...
void ScriptedDesktopCaptureBackend::close() noexcept {
    open_ = false;
    next_step_ = 0;
}

std::optional<DesktopCaptureIssue>
ScriptedDesktopCaptureBackend::validate_image(const DesktopImage& image) const {
    if (image.width == 0 || image.height == 0) {
        return issue(DesktopCaptureOperation::Copy,
                     "scripted desktop image dimensions must be non-zero");
    }
    if (image.width > std::numeric_limits<std::uint32_t>::max() / 4U) {
        return issue(DesktopCaptureOperation::Copy,
                     "scripted desktop image stride exceeds the supported range");
    }

    const auto expected_stride = image.width * 4U;
    if (image.stride != expected_stride) {
        return issue(DesktopCaptureOperation::Copy,
                     "scripted desktop image must use a tightly packed BGRA stride");
    }
    ...
    if (expected_size > std::numeric_limits<std::size_t>::max() ||
        ...
        return issue(DesktopCaptureOperation::Copy,
                     "scripted desktop image buffer size does not match its layout");
    }
    return std::nullopt;
}
