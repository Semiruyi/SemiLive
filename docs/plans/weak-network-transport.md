...
- 冻结当前无反馈 UDP 链路的统计字段；
- Windows 本地开发使用 Clumsy，快速验证随机丢包、延迟、乱序和重复包下的功能行为；
- 正式验收使用流量路径上的 Linux `tc netem`，覆盖随机/突发丢包、延迟、抖动、乱序、
  重复包和带宽限制；
...
本阶段不在 SemiLive 中自研弱网注入器。Clumsy 用于开发和演示，不作为正式性能结论的唯一
依据；正式报告以 `tc netem` 为主要故障源，并通过抓包和接收端统计交叉验证。
...
| 视频恢复 | 损坏AU、等待随机访问时长、RTX恢复点、PLI恢复点、周期IDR恢复点 |
