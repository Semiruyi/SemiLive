...
构造函数等待线程入口确认。若线程创建等必要的线程入口初始化失败，构造失败且不向
Composition 暴露部分可用对象；与一次采集会话相关的 Backend 初始化（包括具体 Backend
需要的平台线程环境）仍放在 `Starting`，通过 `start()` 返回结构化错误。
...
需要 COM 的 Backend 在 `open()` 中初始化对应 Apartment，并在同一线程的 `close()` 中释放。
