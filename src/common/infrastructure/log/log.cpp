...
constexpr char kLoggerName[] = "semilive";
...
constexpr std::string_view kDefaultFilePath = "logs/semilive.log";
constexpr std::string_view kPattern = "%Y-%m-%d %H:%M:%S.%e [%^%l%$] [tid %t] %v";

int to_rank(const Level level) noexcept {
...
spdlog::async_overflow_policy to_overflow_policy(const OverflowPolicy policy) noexcept {
    switch (policy) {
...
std::string_view base_name(const std::string_view path) noexcept {
...
std::tm local_time(const std::time_t value) noexcept {
    std::tm time{};
#if defined(_WIN32)
...
std::string render_payload(const std::string_view tag,
                           const std::source_location& location,
                           const std::string_view message) {
    return fmt::format(
        "[{}] [{}:{}] {}", tag, base_name(location.file_name()), location.line(), message);
}

void raw_stderr_write(const Level level,
    ...
    try {
        ...
        const auto local = local_time(time_value);
        ...
        std::lock_guard lock{g_stderr_mutex};
...
bool running_state(const State state) noexcept {
...
InitResult init(const Config& config) noexcept {
    ...
    try {
        ...
        detail::report_internal_failure("logger init failed", exception.what());
    } catch (...) {
...
void shutdown() noexcept {
    ...
    if (logger) {
        try {
            ...
            detail::report_internal_failure("logger flush during shutdown failed", exception.what());
        } catch (...) {
...
void flush() noexcept {
    ...
    try {
        ...
        detail::report_internal_failure("logger flush failed", exception.what());
    } catch (...) {
...
bool should_log(const Level level) noexcept {
    const auto state = g_state.load();
...
void write_formatted(const Level level,
                     const std::string_view tag,
                     const std::source_location& location,
                     const std::string_view message) noexcept {
    try {
        ...
        report_internal_failure("logger write failed", exception.what());
    } catch (...) {
        report_internal_failure("logger write failed", "unknown exception");
    }
}

void report_internal_failure(const std::string_view context,
                             const std::string_view detail) noexcept {
    try {
        raw_stderr_write(Level::Error,
                         "Logger",
