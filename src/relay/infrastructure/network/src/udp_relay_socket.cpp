#include <semilive/relay/infrastructure/network/udp_relay_socket.hpp>

#include <algorithm>
#include <cerrno>
#include <chrono>
#include <climits>
#include <cstring>
#include <expected>
#include <string_view>
#include <utility>

#if defined(_WIN32)
#ifndef NOMINMAX
#define NOMINMAX
#endif
#include <winsock2.h>
#include <ws2tcpip.h>
#else
#include <arpa/inet.h>
#include <netinet/in.h>
#include <sys/select.h>
#include <sys/socket.h>
#include <unistd.h>
#endif

namespace semilive::relay::infra::network {
namespace {

constexpr std::size_t maximum_udp_payload_size = 65'507;

[[nodiscard]] UdpRelaySocketIssue issue(
    const UdpRelaySocketOperation operation,
    const std::int64_t native_code,
    std::string message) {
    return {operation, native_code, std::move(message)};
}

#if defined(_WIN32)
using NativeSocket = SOCKET;
using SocketLength = int;
constexpr NativeSocket invalid_socket = INVALID_SOCKET;

[[nodiscard]] std::int64_t current_socket_error() noexcept {
    return static_cast<std::int64_t>(WSAGetLastError());
}

[[nodiscard]] bool interrupted(const std::int64_t error) noexcept {
    return error == WSAEINTR;
}

void close_native_socket(const NativeSocket socket) noexcept {
    if (socket != invalid_socket) {
        static_cast<void>(closesocket(socket));
    }
}
#else
using NativeSocket = int;
using SocketLength = socklen_t;
constexpr NativeSocket invalid_socket = -1;

[[nodiscard]] std::int64_t current_socket_error() noexcept {
    return static_cast<std::int64_t>(errno);
}

[[nodiscard]] bool interrupted(const std::int64_t error) noexcept {
    return error == EINTR;
}

void close_native_socket(const NativeSocket socket) noexcept {
    if (socket != invalid_socket) {
        static_cast<void>(::close(socket));
    }
}
#endif

struct ParsedEndpoint {
    sockaddr_storage storage{};
    SocketLength size = 0;
    int family = AF_UNSPEC;
};

[[nodiscard]] bool endpoints_overlap(
    const ParsedEndpoint& bind_endpoint,
    const std::uint16_t bind_port,
    const ParsedEndpoint& forward_endpoint,
    const std::uint16_t forward_port) noexcept {
    if (bind_port != forward_port ||
        bind_endpoint.family != forward_endpoint.family) {
        return false;
    }
    if (bind_endpoint.family == AF_INET) {
        const auto* bind_ipv4 =
            reinterpret_cast<const sockaddr_in*>(&bind_endpoint.storage);
        const auto* forward_ipv4 =
            reinterpret_cast<const sockaddr_in*>(&forward_endpoint.storage);
        return bind_ipv4->sin_addr.s_addr == htonl(INADDR_ANY) ||
               bind_ipv4->sin_addr.s_addr ==
                   forward_ipv4->sin_addr.s_addr;
    }
    if (bind_endpoint.family == AF_INET6) {
        const auto* bind_ipv6 =
            reinterpret_cast<const sockaddr_in6*>(&bind_endpoint.storage);
        const auto* forward_ipv6 =
            reinterpret_cast<const sockaddr_in6*>(&forward_endpoint.storage);
        return IN6_IS_ADDR_UNSPECIFIED(&bind_ipv6->sin6_addr) != 0 ||
               std::memcmp(&bind_ipv6->sin6_addr,
                           &forward_ipv6->sin6_addr,
                           sizeof(in6_addr)) == 0;
    }
    return false;
}

[[nodiscard]] std::expected<ParsedEndpoint, UdpRelaySocketIssue>
parse_endpoint(const std::string_view address,
               const std::uint16_t port,
               const UdpRelaySocketOperation operation,
               const std::string_view label) {
    if (address.empty()) {
        return std::unexpected{issue(
            operation, 0, std::string{label} + " address must not be empty")};
    }

    const std::string address_text{address};
    ParsedEndpoint endpoint;
    sockaddr_in ipv4{};
    if (inet_pton(AF_INET, address_text.c_str(), &ipv4.sin_addr) == 1) {
        ipv4.sin_family = AF_INET;
        ipv4.sin_port = htons(port);
        endpoint.size = static_cast<SocketLength>(sizeof(ipv4));
        endpoint.family = AF_INET;
        std::memcpy(&endpoint.storage, &ipv4, sizeof(ipv4));
        return endpoint;
    }

    sockaddr_in6 ipv6{};
    if (inet_pton(AF_INET6, address_text.c_str(), &ipv6.sin6_addr) == 1) {
        ipv6.sin6_family = AF_INET6;
        ipv6.sin6_port = htons(port);
        endpoint.size = static_cast<SocketLength>(sizeof(ipv6));
        endpoint.family = AF_INET6;
        std::memcpy(&endpoint.storage, &ipv6, sizeof(ipv6));
        return endpoint;
    }

    return std::unexpected{issue(
        operation, 0,
        std::string{label} +
            " address must be a numeric IPv4 or IPv6 address")};
}

[[nodiscard]] std::expected<void, UdpRelaySocketIssue> validate_config(
    const UdpRelaySocketConfig& config) {
    if (config.forward_port == 0) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, 0,
            "UDP forward port must be in 1..65535")};
    }
    if (config.maximum_datagram_bytes == 0 ||
        config.maximum_datagram_bytes > maximum_udp_payload_size) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, 0,
            "UDP maximum datagram size must be in 1..65507")};
    }
    if (config.receive_buffer_bytes == 0 ||
        config.receive_buffer_bytes > static_cast<std::size_t>(INT_MAX)) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, 0,
            "UDP receive buffer size must be in 1..2147483647")};
    }
    return {};
}

[[nodiscard]] std::expected<std::size_t, UdpRelaySocketIssue>
configure_receive_buffer(const NativeSocket socket,
                         const std::size_t requested_bytes) {
    const auto requested = static_cast<int>(requested_bytes);
#if defined(_WIN32)
    const auto set_result = setsockopt(
        socket, SOL_SOCKET, SO_RCVBUF,
        reinterpret_cast<const char*>(&requested),
        static_cast<int>(sizeof(requested)));
#else
    const auto set_result = setsockopt(
        socket, SOL_SOCKET, SO_RCVBUF, &requested,
        static_cast<socklen_t>(sizeof(requested)));
#endif
    if (set_result != 0) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, current_socket_error(),
            "failed to configure UDP relay receive buffer")};
    }

    int actual = 0;
#if defined(_WIN32)
    int option_size = static_cast<int>(sizeof(actual));
    const auto get_result = getsockopt(
        socket, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<char*>(&actual),
        &option_size);
#else
    socklen_t option_size = static_cast<socklen_t>(sizeof(actual));
    const auto get_result = getsockopt(
        socket, SOL_SOCKET, SO_RCVBUF, &actual, &option_size);
#endif
    if (get_result != 0 || actual <= 0) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, current_socket_error(),
            "failed to query UDP relay receive buffer")};
    }
    return static_cast<std::size_t>(actual);
}

[[nodiscard]] std::expected<std::pair<std::string, std::uint16_t>,
                            UdpRelaySocketIssue>
bound_endpoint(const NativeSocket socket) {
    sockaddr_storage storage{};
    SocketLength size = static_cast<SocketLength>(sizeof(storage));
    if (getsockname(socket, reinterpret_cast<sockaddr*>(&storage), &size) !=
        0) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, current_socket_error(),
            "failed to query UDP relay endpoint")};
    }

    const void* address = nullptr;
    std::uint16_t port = 0;
    if (storage.ss_family == AF_INET) {
        const auto* ipv4 = reinterpret_cast<const sockaddr_in*>(&storage);
        address = &ipv4->sin_addr;
        port = ntohs(ipv4->sin_port);
    } else if (storage.ss_family == AF_INET6) {
        const auto* ipv6 = reinterpret_cast<const sockaddr_in6*>(&storage);
        address = &ipv6->sin6_addr;
        port = ntohs(ipv6->sin6_port);
    } else {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, 0,
            "UDP relay bound an unsupported address family")};
    }

    char address_text[INET6_ADDRSTRLEN]{};
    if (inet_ntop(storage.ss_family, address, address_text,
                  static_cast<SocketLength>(sizeof(address_text))) ==
        nullptr) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, current_socket_error(),
            "failed to format UDP relay endpoint")};
    }
    return std::pair<std::string, std::uint16_t>{address_text, port};
}

[[nodiscard]] timeval select_timeout(
    const std::chrono::microseconds remaining) noexcept {
    using Seconds = std::chrono::seconds;
    const auto seconds = std::chrono::duration_cast<Seconds>(remaining);
    const auto bounded_seconds = std::min<std::int64_t>(
        seconds.count(), static_cast<std::int64_t>(LONG_MAX));
    const auto consumed = Seconds{bounded_seconds};
    const auto microseconds = remaining - consumed;
    return {static_cast<long>(bounded_seconds),
            bounded_seconds == seconds.count()
                ? static_cast<long>(microseconds.count())
                : 0L};
}

}  // namespace

struct UdpRelaySocket::Impl {
    ~Impl() {
        close();
    }

    [[nodiscard]] UdpRelaySocketOpenResult open(
        const UdpRelaySocketConfig& config);
    [[nodiscard]] UdpRelaySocketReceiveResult receive_for(
        std::chrono::milliseconds timeout);
    [[nodiscard]] UdpRelaySocketSendResult send(
        std::span<const std::byte> datagram);
    void close() noexcept;

    NativeSocket input_socket = invalid_socket;
    NativeSocket output_socket = invalid_socket;
    std::size_t maximum_datagram_bytes = 0;
    std::vector<std::byte> receive_buffer =
        std::vector<std::byte>(maximum_udp_payload_size);
#if defined(_WIN32)
    bool winsock_started = false;
#endif
};

UdpRelaySocketOpenResult UdpRelaySocket::Impl::open(
    const UdpRelaySocketConfig& config) {
    if (input_socket != invalid_socket || output_socket != invalid_socket) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::State, 0,
            "UDP relay socket can only open from the closed state")};
    }
    if (auto valid = validate_config(config); !valid) {
        return std::unexpected{std::move(valid.error())};
    }

#if defined(_WIN32)
    WSADATA data{};
    const auto startup_result = WSAStartup(MAKEWORD(2, 2), &data);
    if (startup_result != 0) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open,
            static_cast<std::int64_t>(startup_result),
            "failed to initialize Winsock for UDP relay")};
    }
    winsock_started = true;
#endif

    auto bind_endpoint = parse_endpoint(
        config.bind_address, config.bind_port,
        UdpRelaySocketOperation::Open, "UDP bind");
    auto forward_endpoint = parse_endpoint(
        config.forward_address, config.forward_port,
        UdpRelaySocketOperation::Open, "UDP forward");
    if (!bind_endpoint || !forward_endpoint) {
        auto error = !bind_endpoint ? std::move(bind_endpoint.error())
                                    : std::move(forward_endpoint.error());
        close();
        return std::unexpected{std::move(error)};
    }
    if (endpoints_overlap(*bind_endpoint, config.bind_port,
                          *forward_endpoint, config.forward_port)) {
        close();
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, 0,
            "input and forward UDP endpoints must be different")};
    }

    input_socket =
        ::socket(bind_endpoint->family, SOCK_DGRAM, IPPROTO_UDP);
    if (input_socket == invalid_socket) {
        const auto code = current_socket_error();
        close();
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, code,
            "failed to create UDP relay input socket")};
    }
    auto receive_buffer_size = configure_receive_buffer(
        input_socket, config.receive_buffer_bytes);
    if (!receive_buffer_size) {
        auto error = std::move(receive_buffer_size.error());
        close();
        return std::unexpected{std::move(error)};
    }
    if (::bind(input_socket,
               reinterpret_cast<const sockaddr*>(&bind_endpoint->storage),
               bind_endpoint->size) != 0) {
        const auto code = current_socket_error();
        close();
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, code,
            "failed to bind UDP relay input socket")};
    }

    output_socket =
        ::socket(forward_endpoint->family, SOCK_DGRAM, IPPROTO_UDP);
    if (output_socket == invalid_socket) {
        const auto code = current_socket_error();
        close();
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, code,
            "failed to create UDP relay output socket")};
    }
    if (::connect(
            output_socket,
            reinterpret_cast<const sockaddr*>(&forward_endpoint->storage),
            forward_endpoint->size) != 0) {
        const auto code = current_socket_error();
        close();
        return std::unexpected{issue(
            UdpRelaySocketOperation::Open, code,
            "failed to connect UDP relay output socket")};
    }

    auto endpoint = bound_endpoint(input_socket);
    if (!endpoint) {
        auto error = std::move(endpoint.error());
        close();
        return std::unexpected{std::move(error)};
    }
    maximum_datagram_bytes = config.maximum_datagram_bytes;
    return UdpRelaySocketInfo{endpoint->first, endpoint->second,
                              maximum_datagram_bytes,
                              *receive_buffer_size};
}

UdpRelaySocketReceiveResult UdpRelaySocket::Impl::receive_for(
    const std::chrono::milliseconds timeout) {
    if (input_socket == invalid_socket) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::State, 0,
            "UDP relay socket must be open before receiving")};
    }
    if (timeout < std::chrono::milliseconds::zero()) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Receive, 0,
            "UDP relay receive timeout must not be negative")};
    }

    using Clock = std::chrono::steady_clock;
    const auto deadline = Clock::now() + timeout;
    bool first_attempt = true;
    while (first_attempt || Clock::now() < deadline) {
        first_attempt = false;
        const auto remaining =
            timeout == std::chrono::milliseconds::zero()
                ? std::chrono::microseconds::zero()
                : std::chrono::ceil<std::chrono::microseconds>(
                      std::max(Clock::duration::zero(),
                               deadline - Clock::now()));
        auto native_timeout = select_timeout(remaining);
        fd_set readable;
        FD_ZERO(&readable);
        FD_SET(input_socket, &readable);
#if defined(_WIN32)
        const auto selected =
            select(0, &readable, nullptr, nullptr, &native_timeout);
        if (selected == SOCKET_ERROR) {
#else
        const auto selected = select(input_socket + 1, &readable, nullptr,
                                     nullptr, &native_timeout);
        if (selected == -1) {
#endif
            const auto code = current_socket_error();
            if (interrupted(code)) {
                continue;
            }
            return std::unexpected{issue(
                UdpRelaySocketOperation::Receive, code,
                "failed while waiting for a UDP relay datagram")};
        }
        if (selected == 0) {
            if (Clock::now() < deadline) {
                continue;
            }
            return std::optional<std::vector<std::byte>>{};
        }

#if defined(_WIN32)
        const auto received = ::recvfrom(
            input_socket, reinterpret_cast<char*>(receive_buffer.data()),
            static_cast<int>(receive_buffer.size()), 0, nullptr, nullptr);
        if (received == SOCKET_ERROR) {
#else
        const auto received = ::recvfrom(
            input_socket, receive_buffer.data(), receive_buffer.size(), 0,
            nullptr, nullptr);
        if (received == -1) {
#endif
            const auto code = current_socket_error();
            if (interrupted(code)) {
                continue;
            }
            return std::unexpected{issue(
                UdpRelaySocketOperation::Receive, code,
                "failed to receive UDP relay datagram")};
        }

        const auto size = static_cast<std::size_t>(received);
        if (size > maximum_datagram_bytes) {
            return std::unexpected{issue(
                UdpRelaySocketOperation::Receive, 0,
                "received UDP datagram exceeds the configured maximum")};
        }
        const auto end = receive_buffer.begin() +
                         static_cast<std::ptrdiff_t>(size);
        return std::optional<std::vector<std::byte>>{
            std::in_place, receive_buffer.begin(), end};
    }
    return std::optional<std::vector<std::byte>>{};
}

UdpRelaySocketSendResult UdpRelaySocket::Impl::send(
    const std::span<const std::byte> datagram) {
    if (output_socket == invalid_socket) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::State, 0,
            "UDP relay socket must be open before sending")};
    }
    if (datagram.size() > maximum_udp_payload_size) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Send, 0,
            "UDP relay datagram exceeds the payload limit")};
    }

#if defined(_WIN32)
    const auto sent = ::send(
        output_socket, reinterpret_cast<const char*>(datagram.data()),
        static_cast<int>(datagram.size()), 0);
    if (sent == SOCKET_ERROR) {
#else
    constexpr int send_flags =
#if defined(MSG_NOSIGNAL)
        MSG_NOSIGNAL;
#else
        0;
#endif
    const auto sent = ::send(output_socket, datagram.data(), datagram.size(),
                             send_flags);
    if (sent == -1) {
#endif
        return std::unexpected{issue(
            UdpRelaySocketOperation::Send, current_socket_error(),
            "failed to forward UDP relay datagram")};
    }
    if (static_cast<std::size_t>(sent) != datagram.size()) {
        return std::unexpected{issue(
            UdpRelaySocketOperation::Send, 0,
            "UDP relay sent an incomplete datagram")};
    }
    return {};
}

void UdpRelaySocket::Impl::close() noexcept {
    close_native_socket(input_socket);
    close_native_socket(output_socket);
    input_socket = invalid_socket;
    output_socket = invalid_socket;
    maximum_datagram_bytes = 0;
#if defined(_WIN32)
    if (winsock_started) {
        static_cast<void>(WSACleanup());
        winsock_started = false;
    }
#endif
}

UdpRelaySocket::UdpRelaySocket() : impl_{std::make_unique<Impl>()} {}

UdpRelaySocket::~UdpRelaySocket() = default;

UdpRelaySocketOpenResult UdpRelaySocket::open(
    const UdpRelaySocketConfig& config) {
    return impl_->open(config);
}

UdpRelaySocketReceiveResult UdpRelaySocket::receive_for(
    const std::chrono::milliseconds timeout) {
    return impl_->receive_for(timeout);
}

UdpRelaySocketSendResult UdpRelaySocket::send(
    const std::span<const std::byte> datagram) {
    return impl_->send(datagram);
}

void UdpRelaySocket::close() noexcept {
    impl_->close();
}

}  // namespace semilive::relay::infra::network
